...
#define IPREASS_TIMEOUT 3
static struct sip_reass *ip_reass_list = NULL;
struct skbuff *sip_reassemble(struct skbuff* skb)
{
	...
	if(!found)										/*没有找到合适的分组链?*/
	{
		...
		ip_reass_list = ipr;	
		memcpy(&ipr->iphdr, skb->nh.raw, sizeof(IPHDR_LEN));/*复制IP的数据头部,便于之后的分片匹配*/
	}else{										/*找到合适的分组链*/
...
#define FRAG_LENGTH(iph) (ntohs(iph->tot_len) - IPHDR_LEN)
		...
		skb_prev=skb_cur,skb_cur = skb_cur->next)
	{
		if(skb_prev !=NULL)								/*不是第一个分片*/
		{
			...
			{				
				...
				{									/*计算当前链的数据长度修改值*/
					__u16 modify = FRAG_OFFSET(skb_cur->nh.iph) - offset + IPHDR_LEN;
				...
				{									/*计算前面数据长度的更改之*/
					__u16 modify = FRAG_OFFSET(skb_prev->nh.iph) - offset + IPHDR_LEN;
	...
	if(length == ipr->datagram_len )						/*分组全部接收到?*/
	{
		...
		memcpy(skb->nh.raw, & ipr->iphdr, sizeof(ipr->iphdr));	/*向新数据结构中复制IP头*/
		skb->nh.iph->tot_len = htons(ipr->datagram_len);		/*新结构中的tot_len*/
		
		for(skb_prev=skb_cur=ipr->skb;skb_cur != NULL;)		/*遍历重组数据链*/
		{
			int size = skb_cur->end - skb_cur->tail;			/*计算复制数据源的长度*/
			pos = skb_put(skb, size);						/*计算复制目的地址位置*/
			memcpy(pos, 								/*将一个分片复制到新结构中*/
				skb_cur->tail, 
...
struct skbuff * ip_frag(struct net_device *dev, struct skbuff *skb)
{
	...
	for(i = 0,skb->tail = skb->head; i<frag_num;i++)
	{
		if(i ==0){													/*第一个分片*/
			...
			memcpy(skb_t->head, skb->head, mtu);						/*复制数据*/
			...
			memcpy(skb_t->head, skb->head, ETH_HLEN + IPHDR_LEN);	/*复制数据*/
